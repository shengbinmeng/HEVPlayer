...
JNIEnv* getJNIEnv()
{
    ...
    if (ret == JNI_OK) {
        ...
        if(attachSuccess == 0) {
			...
			LOGE("AttachCurrentThread(void** penv, void* args) was not successful. This may be due to the thread being attached already to another JVM instance\n");
			return NULL;
		}
    } else {
    	LOGE("Failed to obtain JNIEnv, return: %d", ret);
    }
    return env;
}
