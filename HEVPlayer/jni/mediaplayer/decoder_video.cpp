...
DecoderVideo::DecoderVideo(AVStream* stream) : Decoder(stream)
{
...
double DecoderVideo::synchronize(AVFrame *frame, double pts)
{
	double frameDelay;
	if (pts != 0) {
...
{
	//LOGI("process a video packet  %.3f", getms());
	int	gotFrame = 0;
    double pts = 0;
	// Decode video frame
	...
	//LOGD("width: %d, height: %d",mStream->codec->width,mStream->codec->height);
	...
	if (gotFrame) {
		...
		//LOGI("a frame completed %.3f", getms());
		onDecoded(mFrame, pts);
		//LOGI("a frame output %.3f", getms());
	}
...
bool DecoderVideo::decode(void* ptr)
{
    ...
    while (mRunning) {
		...
		//LOGI("out queue a video packet. Number:%d", packets());
        if (!process(&packet)) {
            mRunning = false;
            return false;
        }
